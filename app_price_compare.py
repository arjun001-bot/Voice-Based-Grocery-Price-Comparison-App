...
st.markdown("Type or paste or press **Record** and speak your whole list. ")
...
def load_all_store_dfs()->Dict[str,pd.DataFrame]:
    return{
        ...
        "Nofrill":_read_csv_with_fallback(NOFRILLS_CSV),
        "Walmart":_read_csv_with_fallback(WALMART_CSV)
    }
def currency(v):
    return f"${v:,.2f}" if pd.notnull(v) else "-"
def listen_once(timeout:int=3,phrase_time_limit:int=8):
    r=sr.Recognizer()
    try:
...
def greedy_extract_from_transcript(transcript: str, choices: List[str], max_items: int, score_cutoff: int = 78) -> List[str]:
    ...
    for window in range(min(6,n_tokens),0,-1):
        ...
        while(i+window)<=n_tokens and len(found)<max_items:
            canditate=" ".join(tokens[i:i+window])
            res=process.extractOne(canditate,choices,scorer=fuzz.WRatio)
...
def top_match_in_store(name:str, df:pd.DataFrame,score_cutoff:int=60):
    ...
    choices=df['name'].dropna().astype(str).to_list()
...
with left:
    st.write("Enter a grocery list (comma/newline/'and' separated) or record your list.")
